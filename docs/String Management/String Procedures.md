...
Checking and fixing broken surrogates is diabled by default for speed reasons. If you want to enable checking, use `"#define __AFX2_CHECKSURROGATES__"` (without the quotes) at the beginning of your application, before including the files of this framework. Surrogate checking and fixing is centralized in te internal **AppendBuffer** method of the `DWSTRING` class.
